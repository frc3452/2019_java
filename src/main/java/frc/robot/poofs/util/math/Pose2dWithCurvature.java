...
public class Pose2dWithCurvature implements IPose2d<Pose2dWithCurvature>, ICurvature<Pose2dWithCurvature> {
    ...
    public Pose2dWithCurvature interpolate(final Pose2dWithCurvature other, double x) {
        return new Pose2dWithCurvature(getPose().interpolate(other.getPose(), x),
                GZUtil.interpolate(getCurvature(), other.getCurvature(), x),
                GZUtil.interpolate(getDCurvatureDs(), other.getDCurvatureDs(), x));
    }
    ...
    public boolean equals(final Object other) {
        ...
        return getPose().equals(p2dwc.getPose()) && GZUtil.epsilonEquals(getCurvature(), p2dwc.getCurvature()) && GZUtil.epsilonEquals(getDCurvatureDs(), p2dwc.getDCurvatureDs());
    }
