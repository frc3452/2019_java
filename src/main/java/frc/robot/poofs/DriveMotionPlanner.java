...
import frc.robot.poofs.physics.DCMotorTransmission;
import frc.robot.poofs.physics.DifferentialDrive;
import frc.robot.poofs.trajectory.DistanceView;
import frc.robot.poofs.trajectory.PurePursuitController;
import frc.robot.poofs.trajectory.Trajectory;
import frc.robot.poofs.trajectory.TrajectoryIterator;
import frc.robot.poofs.trajectory.TrajectorySamplePoint;
import frc.robot.poofs.trajectory.TrajectoryUtil;
import frc.robot.poofs.trajectory.timing.DifferentialDriveDynamicsConstraint;
import frc.robot.poofs.trajectory.timing.TimedState;
import frc.robot.poofs.trajectory.timing.TimingConstraint;
...
public class DriveMotionPlanner {
    ...
    public DriveMotionPlanner() {
        final DCMotorTransmission transmission = new DCMotorTransmission(
                1.0 / kPoofs.kDriveKv,
                Units.inches_to_meters(kPoofs.kDriveWheelRadiusInches) * Units.inches_to_meters(kPoofs
                        .kDriveWheelRadiusInches) * kPoofs.kRobotLinearInertia / (2.0 * kPoofs.kDriveKa),
                kPoofs.kDriveVIntercept);
        mModel = new DifferentialDrive(
                kPoofs.kRobotLinearInertia,
                kPoofs.kRobotAngularInertia,
                kPoofs.kRobotAngularDrag,
                Units.inches_to_meters(kPoofs.kDriveWheelDiameterInches / 2.0),
                Units.inches_to_meters(kPoofs.kDriveWheelTrackWidthInches / 2.0 * kPoofs.kTrackScrubFactor),
    ...
    public void setTrajectory(final TrajectoryIterator<TimedState<Pose2dWithCurvature>> trajectory) {
        ...
        for (int i = 0; i < trajectory.trajectory().length(); ++i) {
            if (trajectory.trajectory().getState(i).velocity() > kPoofs.kEpsilon) {
                mIsReversed = false;
                break;
            } else if (trajectory.trajectory().getState(i).velocity() < -kPoofs.kEpsilon) {
                mIsReversed = true;
                break;
            }
        }
    ...
    protected Output updatePurePursuit(DifferentialDrive.DriveDynamics dynamics, Pose2d current_state) {
        double lookahead_time = kPoofs.kPathLookaheadTime;
        ...
        while (actual_lookahead_distance < kPoofs.kPathMinLookaheadDistance &&
        ...
        if (actual_lookahead_distance < kPoofs.kPathMinLookaheadDistance) {
            lookahead_state = new TimedState<>(new Pose2dWithCurvature(lookahead_state.state()
                    .getPose().transformBy(Pose2d.fromTranslation(new Translation2d(
                            (mIsReversed ? -1.0 : 1.0) * (kPoofs.kPathMinLookaheadDistance -
        ...
        DifferentialDrive.ChassisState adjusted_velocity = new DifferentialDrive.ChassisState();
        // Feedback on longitudinal error (distance).
        adjusted_velocity.linear = dynamics.chassis_velocity.linear + kPoofs.kPathKX * Units.inches_to_meters
                (mError.getTranslation().x());
